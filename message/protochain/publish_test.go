...
func TestPublish(t *testing.T) {
	...
	p, err := protochain.NewPublisher(testStore, userFeeds, testKP)
	r.NoError(err)
	...
	for i, tc := range testContent {
		...
		if i == 0 {
			...
			// compare reference
			prevRef, err := evt.Previous.GetRef(protochain.RefType_MessageRef)
			r.NoError(err)
...
func TestMultipleFeeds(t *testing.T) {
	...
	alice, err := protochain.NewPublisher(testStore, userFeeds, keypairAlice)
	r.NoError(err)
	bob, err := protochain.NewPublisher(testStore, userFeeds, keypairBob)
	r.NoError(err)
